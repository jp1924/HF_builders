...
class KconfSpeech(GeneratorBasedBuilder):
    ...
    def _generate_examples(self, filepath: List[Path], split: str):
        ...
        for source_zip, label_zip in zip(source_ls, label_ls):
            ...
            for _id, wav_ls in source_dict.items():
                meta = [x for x in label_dict[_id] if "json" in x.filename][0]
                text_ls = [x for x in label_dict[_id] if "txt" in x.filename]

                meta = json.loads(label_zip.open(meta).read().decode("utf-8"))
                ...
                for txt_info, wav_info in zip(text_ls, wav_ls):
                    ...
                    audio = source_zip.open(wav_info).read()
