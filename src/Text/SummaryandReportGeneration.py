...
class MultilingualCorpusInFinancialSector(GeneratorBasedBuilder):
    ...
    def _info(self):
        if self.config.name == "Translation":
            features = Features(
                {
                    "id": Value("string"),
                    "translation": Translation(languages=["Kor", "Eng", "Jpn", "Vie", "Ind", "Cmn"]),
                    "metadata": {
                        "doc_info": {
                            "source": Value("string"),
    ...
    def _translation_generate_examples(
        ...
        for lbl_zip in lbl_zip_ls:
            lbl_info_ls = [info for info in lbl_zip.infolist() if not info.is_dir() and "json" in info.filename]
            for lbl_info in lbl_info_ls:
                ...
                for sents in labels["sents"]:
                    ...
                    translation = {lang: None for lang in lang_map.values() if lang != trg_lang}
                    translation = {
                        **{
    ...
    def _corpus_generate_examples(
        ...
        for lbl_zip in lbl_zip_ls:
            ...
            if corpus_type != "한국어" and (corpus_type not in Path(lbl_zip.filename).stem):
                continue

            lbl_info_ls = natsorted(
                [info for info in lbl_zip.infolist() if not info.is_dir() and "json" in info.filename],
            ...
            for label_info in lbl_info_ls:
                ...
                sentence_ls = list()
                for sents in labels["sents"]:
                    corpus = sents["source_cleaned"] if self.config.name == "Kor-Corpus" else sents["mtpe"]
                    sentence_ls.append(corpus.strip())
    ...
    def _original_generate_examples(self, file_ls: List[Path], split: str) -> Generator:
        ...
        label_info_ls = natsorted(
            [info for info in label_zip.infolist() if not info.is_dir() and "json" in info.filename],
            key=lambda x: x.filename,
        )
