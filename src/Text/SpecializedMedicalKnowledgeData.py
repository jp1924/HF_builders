...
_DATANAME = "SpecializedMedicalKnowledgeData"
DATASET_SIZE = 0.1926  # GB
_DESCRIPTION = """● 국내 Big 5 대형병원 중 총 4개 병원 (서울성모병원, 삼성서울병원, 서울대학교병원, 세브란스병원), 국내 최대규모의 공공병원인 서울특별시 보라매병원에서 참여하여, 고품질의 전문 의학지식 관련 말뭉치 및 질의응답 데이터셋을 확보
● 임상학적 근거가 분명한 전문 의학지식(질병의 원인, 진단, 치료, 관리, 예방, 최신지견 등)을 내포한 영어/한글 전문 의학 말뭉치 1억 토큰 구축/개방
● 향후 의학지식에 대한 전문적 자연어처리 태스크에 대응할 수 있는 전문의학 지식 질의응답 1.5만 쌍 (10만 토큰) 구축/개방
● 진료 보조, 예방 및 건강증진, 의료 연구 및 교육에 있어 본 사업의 개방 데이터와 AI 모델은 의료분야 생성형 AI의 근간이 될 것
...
class SpecializedMedicalKnowledgeData(GeneratorBasedBuilder):
